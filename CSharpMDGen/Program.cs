...
namespace CSharpMDGen
{
    class CSharpMDGen
    {
        public static void Main()
        {
            ...
            var pathToConfig = "./MDGenConfig.xml";
            var fileNamesMap = ConfigParser.LoadFileNames(pathToConfig);

            foreach (var entry in fileNamesMap)
                Console.WriteLine($"docId: {entry.Key}, fileName: {entry.Value}");
    ...
    {
        public static Dictionary<string, string> LoadFileNames(string configPath)
        {
            ...
            try
            {
                ...
                var fileNameElements = doc.Descendants("FileNames")
                                          .Descendants("file");

                foreach (var fileElem in fileNameElements)
                {
                    string docId = fileElem.Attribute("docId")?.Value;
                    string fileName = fileElem.Value?.Trim();

                    if (!string.IsNullOrWhiteSpace(docId) && !string.IsNullOrWhiteSpace(fileName))
                    {
                        fileNames[docId] = fileName;
                    }
                }
            }
            catch (Exception ex)
